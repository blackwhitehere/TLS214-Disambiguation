...
from	tls214_extracted_patterns as a
join	(
		select new_id, rtrim(dbo.RegexReplace(dbo.RegexReplace(npl_biblio, @pattern,''),'\s{2,}','')) as bib_numeric
		from [dbo].[sample_unique]
		where dbo.IsMatchValue(npl_biblio, @pattern) <> ''
		)
		as b on a.new_id = b.new_id
...
from	tls214_extracted_patterns as a
join	(
		select new_id, lower(ltrim(rtrim(dbo.RegexReplace(dbo.RegexReplace(npl_biblio, @pattern,''),'\s{2,}' ,'')))) as bib_alphabetic
		from [dbo].[sample_unique]
		)
		as b on a.new_id = b.new_id
go
...
from	tls214_extracted_patterns as a
join	(
		select new_id, lower(ltrim(rtrim(dbo.RegexReplace(dbo.RegexReplace(npl_biblio, @pattern,''),'\s{2,}' ,'')))) as bib_alphanumeric
		from [dbo].[sample_unique]
		)
		as b on a.new_id = b.new_id
go
